...
class BlackBoxTwoGroupsModel:
    def __init__(self, X, y, fdr=0.1, num_alt_bins=220,
        ...
        if estimate_null:
            ...
            print(f'Empirical null: N({mu0:.2f}, {sigma0:.2f}^2)')
        else:
            mu0, sigma0 = 0., 1.
    ...
    def train(self, model_fn=None, lasso=0, l2=1e-1, lr=3e-4, num_epochs=250,
                    batch_size=None, num_folds=3, val_pct=0.1, verbose=False, folds=None,
                    weight_decay=0, random_restarts=1, save_dir='/tmp/',
        ...
        if batch_size is None:
            batch_size = int(max(10,min(100,np.round(self.X.shape[0] / 20.))))
            ...
            for restart in range(random_restarts):
                ...
                for epoch in range(num_epochs):
                    ...
                    for batch_idx, batch in enumerate(batches(train_indices, batch_size, shuffle=False)):
                        ...
                        regularized_loss = loss + lasso * (concentrations-1).mean() + l2 * ((concentrations-1)**2).mean()
    ...
    def predict(self, X, y=None, models=None, batch_size=100):
        ...
        P0 = norm.pdf(y, mu0, sigma0)[:,None]
        P1 = self.alt_dist.pdf(y)[:,None]
